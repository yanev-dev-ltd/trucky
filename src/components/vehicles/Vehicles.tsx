...
import { TextField, CircularProgress, Button, Box, Typography } from '@mui/material'
  ...
  return (
    <Box>
      <Box display='flex' justifyContent='space-between' sx={sx.padding}>
        <Typography variant='h6'><FormattedMessage id='app.Vehicles' /></Typography>
        <TextField
          variant='outlined'
          label={<FormattedMessage id='app.SearchVehicles' />}
          size='small'
        />
        <Button variant='contained' color='primary' startIcon={<Add />}><FormattedMessage id='app.addVehicle' /></Button>
      </Box>
      {Array.isArray(vehicles) && vehicles.length > 0 && <Table columns={columns} data={vehicles} name='vehicles' />}
      {Array.isArray(vehicles) && vehicles.length === 0 && (
        <Box display='flex' justifyContent='center' alignItems='center' flexDirection='column' style={{ height: 'calc(100vh - 54px - 68px)' }}>
          <Typography variant='h5' sx={sx.padding}><FormattedMessage id='app.noVehicles' /></Typography>
        </Box>)}
    </Box>
  )
}
